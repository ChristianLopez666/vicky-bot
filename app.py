...
def _build_template_components(user_components: list = None) -> list:
    """
    Reconstruye componentes de plantilla según estructura real de Meta.
    Mantiene el orden: header -> body -> footer -> buttons
    """
    ...
    # Primero procesar componentes del usuario
    ...
    # Agregar componentes vacíos faltantes para mantener estructura
    if not has_header:
    ...
    if not has_body:
        # Buscar donde insertar body (después de header)
        body_index = 1 if has_header else 0
        final_components.insert(body_index, {
            "type": "body",
            "parameters": []
        })
...
def _detect_template_language(template_name: str, to_number: str) -> str:
    """
    Detección estricta de idioma para plantillas Meta.
    Prueba solo es_MX, es_ES, en_US en ese orden.
    """
    ...
    for lang in posibles:
        ...
        try:
            ...
            if resp.status_code == 200:
                log.info(f"✅ Idioma detectado: {lang} para plantilla {template_name}")
                return lang
    ...
    # Fallback seguro
    log.warning("⚠️ No se pudo detectar idioma, usando en_US como fallback")
    return "en_US"
...
def send_template_message(
    ...
    """
    ENVÍO CORRECTO DE PLANTILLAS META.
    - Reconstrucción automática de componentes
    - Detección estricta de idioma
    - Manejo robusto de errores Meta
    """
    if not (META_TOKEN and WABA_PHONE_ID):
        log.error("❌ WhatsApp API no configurada para plantillas")
        return False

    # 1. Detectar idioma si no se proporciona
    if not language_code:
        language_code = _detect_template_language(template_name, to_number)

    # 2. Reconstruir componentes según estructura Meta
    final_components = _build_template_components(components)
    ...
    # 4. Envío con reintentos y manejo de errores Meta
    meta_errors = {
        100: "Parámetros inválidos",
        131047: "Límite de tasa excedido", 
        132001: "Plantilla no encontrada",
        470: "Acceso no autorizado a plantilla"
    }
    ...
    for attempt in range(3):
        try:
            ...
            if resp.status_code == 200:
                log.info(f"✅ Plantilla {template_name} enviada a {to_number}")
                return True

            # Manejo específico de errores Meta
            error_data = resp.json().get("error", {})
            error_code = error_data.get("code")
            error_message = error_data.get("message", "")
            
            if error_code in meta_errors:
                log.error(f"❌ Error Meta {error_code}: {meta_errors[error_code]} - {error_message}")
                if error_code in [132001, 470]:  # Errores fatales
                    return False
            
            log.warning(f"⚠️ Error plantilla {resp.status_code}: {resp.text[:300]}")
            ...
            log.exception(f"❌ Excepción en send_template_message: {e}")
            if attempt < 2:
                _backoff(attempt)
                continue
            return False
...
# ==========================
# GPT Integration Mejorada
# ==========================
def handle_gpt_query(phone: str, text: str) -> bool:
    """
    Maneja consultas GPT de forma contextual y no invasiva.
    Solo se activa cuando no hay estado activo y no es comando.
    """
    if not (openai and OPENAI_API_KEY):
        return False
        
    try:
        # Limpiar y preparar prompt
        clean_text = text.strip()
        if not clean_text or len(clean_text) < 3:
            return False
            
        # Crear contexto profesional para GPT
        system_context = """
        Eres Vicky, asistente de Inbursa especializada en productos financieros:
        ...
        Si no tienes información específica, sugiere contactar con un asesor.
        Máximo 2 párrafos.
        """
        ...
        if answer:
            ...
            # Ofrecer menú después de responder
            time.sleep(1)
            send_main_menu(phone)
            return True
            
    except Exception as e:
        log.error(f"❌ Error GPT: {e}")
        send_message(phone, "🤖 Disculpa, hubo un error al procesar tu consulta. ¿Puedes intentar de nuevo?")
...
def route_command(phone: str, text: str, match: Optional[Dict[str, Any]]) -> None:
    ...
    if st.startswith("imss_"):
        ...
        # Sin estado y sin comando válido → Intentar GPT
        if not st and len(text) > 10 and not any(word in t for word in ["si", "sí", "no", "hola", "menu"]):
            if handle_gpt_query(phone, text):
                return
...
def webhook_receive():
    try:
        ...
        if mtype == "text":
            ...
            # GPT directo opcional con comando específico
...
def ext_test_send():
    try:
        ...
        if template_name:
            # Enviar plantilla con componentes reconstruidos
            components = data.get("components") or []
            ok = send_template_message(to, template_name, components)
        else:
            # Enviar texto normal
...
# ==========================
# Worker envíos masivos manual (lista explícita) - ACTUALIZADO
# ==========================
def _bulk_send_worker_updated(items: List[Dict[str, Any]]) -> None:
    """
    Worker actualizado para envíos masivos con nueva lógica de plantillas.
    """
    ok = 0
    fail = 0
    for i, item in enumerate(items, 1):
        try:
            ...
            # Validación mejorada
            if not to:
                log.warning(f"⏭️ Item {i} inválido: missing 'to'")
                fail += 1
                continue
            ...
            sent = False
            if template_name:
                # Usar nueva función de plantillas con componentes reconstruidos
...
def ext_send_promo():
    ...
    try:
        ...
        if not isinstance(items, list):
            # Si viene template_name directamente, crear un array de un elemento
            if "template_name" in data or "template" in data:
                ...
                return jsonify(
                    {
                        "queued": False,
                        "error": "Se requiere lista 'items' con mensajes o campos directos 'template_name'/'to'",
                    }
                ), 400
        ...
        if not items:
            return jsonify(
                {
                    "queued": False,
                    "error": "Lista 'items' vacía o sin campos requeridos",
                }
            ), 400
...
# ==========================
# Envío masivo SECOM desde Sheets (WAPI) con soporte para plantillas - ACTUALIZADO
# ==========================
def _bulk_send_from_sheets_worker_updated(
    ...
    ACTUALIZADO: Usa nueva lógica de plantillas Meta.
    """
    if not (google_ready and sheets and SHEETS_ID_LEADS and SHEETS_TITLE_LEADS):
    ...
    try:
        ...
        for offset, row in enumerate(rows, start=2):
            ...
            # PRIORIDAD: Plantilla Meta con nueva lógica
            if template_name:
                sent = send_template_message(to, template_name, components)
...
def ext_send_promo_secom():
    ...
    ACTUALIZADO: Usa nueva lógica de plantillas.
    """
    try:
        if not (META_TOKEN and WABA_PHONE_ID):
