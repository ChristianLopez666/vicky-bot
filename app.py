...
# Python 3.11+
# ------------------------------------------------------------
# CORRECIONES IMPLEMENTADAS:
# 1. ✅ Mejor detección de consultas RAG para preguntas sobre diferencias entre pólizas
# 2. ✅ No más respuestas repetitivas - ahora detecta consultas informativas
# 3. ✅ Flujo contextual mejorado
# ------------------------------------------------------------
...
def auto_start(phone: str, match: Optional[Dict[str, Any]]) -> None:
    ...
    mensaje_auto = (
        "🚗 *Seguro de Auto*\n"
        "Puedo ayudarte con:\n"
        "• Cotización de tu seguro\n" 
        "• Diferencias entre coberturas\n"
        "• Información sobre pólizas\n\n"
        "¿Qué necesitas? Puedes:\n"
        "1. Pedir información sobre coberturas\n" 
        "2. Enviar documentos para cotización\n"
...
def _route_command(phone: str, text: str, match: Optional[Dict[str, Any]]) -> None:
    ...
    # DETECCIÓN MEJORADA DE CONSULTAS INFORMATIVAS
    rag_terms = [
        "diferencia", "diferencias", "qué es", "que es", "cómo funciona", "como funciona",
        "qué cubre", "que cubre", "cuál es", "cual es", "explica", "información",
        "amplia", "limitada", "cobertura", "coberturas", "póliza", "poliza",
        "beneficios", "ventajas", "incluye", "cuesta", "precio", "qué incluye", "que incluye"
    ]
    
    # Detectar consultas informativas (no comandos del menú)
    is_info_query = (
        any(term in t for term in rag_terms) and 
        t not in ["1", "2", "3", "4", "5", "6", "7", "menu", "menú", "inicio", "hola"]
    )
    ...
    if is_info_query:
        ...
        # Si estaba en medio de un flujo, preguntar si quiere continuar
        st = user_state.get(phone, "")
        if st.startswith("auto_"):
            send_message(phone, "\n¿Deseas continuar con la cotización de tu seguro? (sí/no)")
        return
    
    # COMANDOS DEL MENÚ PRINCIPAL (flujo normal)
