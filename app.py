...
META_TOKEN = _get_env("META_TOKEN") or _get_env("WHATSAPP_TOKEN")
WABA_PHONE_ID = _get_env("WABA_PHONE_ID") or _get_env("PHONE_NUMBER_ID")
VERIFY_TOKEN = _get_env("VERIFY_TOKEN")
ADVISOR_NUMBER = _get_env("ADVISOR_NUMBER") or _get_env("ADVISOR_WHATSAPP")
PORT = int(_get_env("PORT", "5000"))

SHEETS_ID_LEADS = _get_env("SHEETS_ID_LEADS") or _get_env("SHEET_ID_SECOM")
SHEETS_TITLE_LEADS = _get_env("SHEETS_TITLE_LEADS") or _get_env("SHEET_TITLE_SECOM", "Prospectos SECOM Auto")
...
def get_state(phone: str) -> str:
    return _user_state.get(normalize_phone(phone), "")
    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code == 200:
                ...
                return True
            log.warning(f"⚠️ Error send_message {r.status_code} {r.text[:300]!r}")
...
def send_template_message(
    ...
    """Envía mensaje de plantilla Meta."""
    if not (META_TOKEN and WPP_API_URL):
    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code == 200:
                ...
                return True
            log.warning(f"⚠️ Error plantilla {template_name} {r.status_code} {r.text[:300]!r}")
            if _should_retry(r.status_code) and attempt < 2:
                _backoff(attempt)
                continue
            return False
...
def match_client_in_sheets(phone: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        for row in rows:
            if len(row) <= idx_wa:
                continue
            tel = normalize_phone(row[idx_wa])
            if tel == target:
...
def _touch_last_inbound(phone: str) -> None:
    ...
    try:
        ...
        for offset, row in enumerate(rows, start=2):
            if len(row) <= idx_wa:
                continue
            if normalize_phone(row[idx_wa]) == target:
...
# === GPT helpers ===
def gpt_answer(phone: str, prompt: str) -> bool:
    if openai and OPENAI_API_KEY:
        try:
            ...
            return send_message(phone, answer)
        except Exception:
...
# === Workers y protección contra ejecución múltiple ===
_worker_reminders_started = False
_worker_reminders_lock = threading.Lock()
...
def _start_reminders_worker() -> None:
    """Protege contra inicialización múltiple en Render, inicia el worker SOLO una vez."""
    global _worker_reminders_started
    with _worker_reminders_lock:
        if _worker_reminders_started or os.environ.get("RENDER_WORKER_STARTED", "") == "1":
            log.info("[REMINDERS] Worker ya iniciado, saltando")
            return
        _worker_reminders_started = True
        os.environ["RENDER_WORKER_STARTED"] = "1"
...
# Invocación bloqueada a una sola ejecución protegida
...
def ext_send_promo():
    ...
    try:
        if not (META_TOKEN and WABA_PHONE_ID):
            return jsonify({"queued": False, "error": "WhatsApp API no configurada"}), 500
        ...
        t = threading.Thread(target=_bulk_send_worker, args=(items,), daemon=True)
        t.start()
        return jsonify(
            {
                "queued": True,
...
def _bulk_send_from_sheets_worker(
    ...
    """Lee la hoja de leads SECOM y envía mensajes uno a uno."""
    if not (google_ready and sheets and SHEETS_ID_LEADS and SHEETS_TITLE_LEADS):
...
# === Flujos SECOM y general de mensajes ===
...
# Flujo principal: Routing de comandos y estados
