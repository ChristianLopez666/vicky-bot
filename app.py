...
def _handle_auto_context_response(phone: str, text: str, match: Dict[str, Any]) -> bool:
    ...
    if t in ("2", "no", "nel") or intent == "negative":
        ...
        send_message(
            phone,
            f"Entendido {nombre}. Para poder recordarte a tiempo, ¿cuál es la *fecha de vencimiento* de tu póliza? (formato AAAA-MM-DD)"
        )
        _notify_advisor(
            f"🔔 AUTO — NO INTERESADO / TIENE SEGURO\n"
            f"WhatsApp: {phone}\n"
            f"Nombre: {nombre}\n"
            f"Respuesta: {text}"
        )
