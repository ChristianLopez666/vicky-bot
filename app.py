# app.py — Vicky SECOM (Versión Completa con Detección de Respuestas Promocionales)
...
def interpret_response(text: str) -> str:
    if not text:
        return "neutral"
    t = text.lower()
    pos = ["sí", "si", "claro", "ok", "de acuerdo", "vale", "afirmativo", "correcto", "interesa", "cuéntame", "info"]
...
# ==========================
# Router helpers - MEJORADO CON DETECCIÓN DE RESPUESTAS PROMOCIONALES
# ==========================
...
def _handle_promotional_response(phone: str, text: str) -> bool:
    """Detecta respuestas a mensajes promocionales y dirige al flujo correcto"""
    text_lower = text.lower().strip()
    
    # Solo procesar si no hay estado activo (es una nueva conversación)
    if user_state.get(phone):
        return False
        
    # Detectar respuestas positivas relacionadas con seguros/auto
    if interpret_response(text) == "positive":
        if any(term in text_lower for term in ["seguro", "auto", "coche", "carro", "vehículo", "automóvil", "cotización"]):
            log.info(f"🎯 Detectada respuesta promocional de {phone} - Iniciando flujo de seguros")
...
def _route_command(phone: str, text: str, match: Optional[Dict[str, Any]]) -> None:
    ...
    # PRIMERO: Verificar si es respuesta a mensaje promocional
    if _handle_promotional_response(phone, text):
        return
