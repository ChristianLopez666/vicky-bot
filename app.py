...
from integrations_gpt import send_whatsapp_message, ask_gpt  # también ask_gpt
        ...
        for entry in entries:
            for change in entry.get("changes", []):
                ...
                for msg in messages:
                    ...
                    try:
                        # Si el mensaje es "menu" o un número → usar menú estático
                        if text_in.lower() == "menu" or text_in.isdigit():
                            reply = route_message(wa_id=wa_id, wa_e164_no_plus=wa_from, text_in=text_in)
                        else:
                            # Para todo lo demás → responder con GPT
                            reply = ask_gpt(text_in)
                    except Exception:
                        logger.exception("Error procesando mensaje para wa_id=%s", wa_id)
